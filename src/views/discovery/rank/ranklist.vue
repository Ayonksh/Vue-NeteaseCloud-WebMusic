<template>
  <transition name="fade">
    <ay-loading :loading="!list.length">
      <ul class="rank-list">
        <li class="img-wrap" @click="toDetail">
          <img v-lazy="getImgUrl(banner, 300, 100)" />
          <div class="play" @click.stop="handlePlay(list)" v-if="id">
            <img :src="require('@/assets/icon/play-red.png')" />
          </div>
        </li>
        <li class="item-wrap" v-for="(item, index) in _list" :key="index">
          <div class="item">
            <div class="left">
